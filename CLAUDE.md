...
### Customer-form source = "Connector" + conditional picker (2026-06-19, hosting-only)
On the **Customers → New Customer** form (`NewLeadPage.tsx`), per Rahul's request: the **Source** dropdown's last option is relabelled **"Connector"** (enum value still `sub_dsa` — display-only, no migration/type churn); the channel-partner picker (the FAC- `/connectors` master) is now **conditional — shown only when Source = Connector** (was always visible), relabelled **"Sourced by Connector"** ("Select connector…" placeholder, "Direct / no Connector" option). Switching away from the Connector source clears any selection (`useEffect` + submit guard on `values.source === 'sub_dsa'`). Display consistency: `sub_dsa` now renders **"Connector"** in `LeadsPage`/`LeadDetailPage` (label + `· Connector:` header)/`MyQueueRow`/`CrmDashboardPage`. `QuickAddConnectorModal` gained an optional **`entityLabel`** prop (default `"Sub DSA"`); NewLeadPage passes `"Connector"` so its "+ New" dialog title/placeholder/button say "Connector" while **AddOpportunityPage keeps "Sub DSA"**. _Deliberate scope: only the Customer-tab lead-source surfaces changed; the case/opportunity/MIS FAC- "Sub DSA" attribution labels (AddOpportunityPage/OpportunityDetailPage) were left as-is — so the same FAC- registry currently reads "Connector" on the customer form but "Sub DSA" on the case form (extend the rename if a single label is wanted)._ tsc clean. No rules/server/index change.
