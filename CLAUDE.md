...
> **Deploy status (2026-06-17):** code is **LIVE in prod** (rev `pulse-api-00049-nwc`, ruleset `6450072d`, `meta_lead_events`/`meta_lead_deadletters` indexes READY). **Wiring (2026-06-18, near-complete):** Meta app **"App leads"** (App ID `1329929825237970`, Finvastra Advisors business, use cases = Capture-leads + Manage-Page; **a DUPLICATE "App leads" also exists — ours is `1329929825237970`; delete the dupe later**). **ALL 4 env vars SET** on Cloud Run (rev **`pulse-api-00057-f56`**): `META_VERIFY_TOKEN` + `META_APP_SECRET` + `META_GRAPH_VERSION=v23.0` + **`META_PAGE_ACCESS_TOKEN`** = a **long-lived Page token for the Finvastra page** (`812414655293252`), obtained via Graph API Explorer → Kumar's user grant → `fb_exchange_token` long-lived → page-by-id token (**the System-User-token path was abandoned** — Meta's new UI hits a circular "No permissions available / assign an app role" wall with no way to install the app on the system user). **Webhook configured + verified live** (handshake 3/3; app subscribed to the Page-object **`leadgen`** field). **Page subscribed**: `POST /{page}/subscribed_apps?subscribed_fields=leadgen` → `{"success":true}` (the read-back GET needs `pages_manage_metadata`, which the token lacks — so verification is via a real lead, not the GET). **REMAINING:** (1) **fire a test lead** (Lead Ads Testing Tool) → confirm it lands in Pipeline → Leads as `source: ADS` (the true end-to-end check); (2) the lead **Instant Form must carry a product question** for Phase-2 routing; (3) **Business Verification** (page shows "Review needed") before Live ad spend; (4) optional: delete the duplicate "App leads"; (5) optional: add `pages_manage_metadata` if subscription management/verify is needed later. `crm2-meta-retry` scheduler already registered. **Gotcha learned:** `/me/accounts` returned EMPTY for Kumar even with Page Full access (the OAuth grant didn't include the page), but **`GET /{page-id}?fields=access_token` worked directly** — use the direct page-by-id query, not the listing. Full runbook: `docs/go-live/PULSE-LEAD-PIPELINE.md`.
