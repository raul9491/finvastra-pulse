...
**crm2 route groups extracted (8). crm2.ts 5721 → 2677 (~53% out).** Remaining money-path (heaviest, one at a time, money gate each, STOP on any non-13/13): **logins · disburse/payout-cycles · mis · recon · jobs.** NOTE: disburse/payout-cycles/MIS may not be cleanly separable (they share the disburse tx + `resolveMapping`/`resolveSlab` + `misRecords`/`payoutCycles` writers) — if a clean cut isn't safe, leave them together rather than force it.
