...
> **Deploy status (2026-06-17):** code is **LIVE in prod** (rev `pulse-api-00049-nwc`, ruleset `6450072d`, `meta_lead_events`/`meta_lead_deadletters` indexes READY). **Wiring started 2026-06-18 (in progress):** the Meta app exists ("App leads", Finvastra Advisors business, 2 use cases = Capture-leads + Manage-Page) and **3 of 4 env vars are now SET** on Cloud Run (rev **`pulse-api-00056-wz5`**): `META_VERIFY_TOKEN` + `META_APP_SECRET` + `META_GRAPH_VERSION=v23.0`. **Webhook handshake verified live 3/3** (`GET …/api/webhooks/meta/leadgen?hub.verify_token=<token>` → echoes challenge; wrong token → 403; unsigned POST → 403). **STILL PENDING to finish:** (1) `META_PAGE_ACCESS_TOKEN` — a long-lived **System User** token (`leads_retrieval`+`pages_manage_metadata`) → set via `gcloud run services update pulse-api --update-env-vars META_PAGE_ACCESS_TOKEN=…`; (2) in Meta: add the webhook callback `https://pulse.finvastra.com/api/webhooks/meta/leadgen` + the verify token, subscribe the **`leadgen`** field; (3) subscribe each Page (`POST /{page-id}/subscribed_apps?subscribed_fields=leadgen`); (4) Instant Form must carry a product question; (5) fire a test lead → `npm run qa:meta:inspect`; (6) Business Verification before Live. `crm2-meta-retry` scheduler already registered. Full runbook: `docs/go-live/PULSE-LEAD-PIPELINE.md`.
...
### Old-CRM "Connector" terminology + conditional source picker (2026-06-19, hosting-only — NEEDS DEPLOY)
