...
### Phase 4 — per-login pipeline rebuild (HEAVIEST; in progress, branch `feature/crm2-phase4-per-login`, **NOT deployed**)
...
- **Remaining (Build #2 / cutover)**: per-login **disburse → per-login payoutCycle + misRecord** (rework the `disburse` + milestone engine to be login-scoped; `payoutCycles`/`misRecords` gain `loginId`); cut the case's own `stage` over to `CaseLevelStage` + the rich Stage-1 form; **relocate payout management to the MIS module**; drop the case **Payout tab → add a "Client-ID data" tab** (decision B); rework the phase-4/5 gates for per-login.
