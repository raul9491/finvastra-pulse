    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-4">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-semibold uppercase tracking-widest mb-1.5" style={{ color: '#8B8B85' }}>Body *</label>
            ...
            <p className="text-xs text-right text-mute mt-1">{body.length} / 2000</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-widest mb-1.5" style={{ color: '#8B8B85' }}>Priority</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-widest mb-1.5" style={{ color: '#8B8B85' }}>Options</label>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-mute">Post and manage company-wide announcements.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center"><p className="text-sm text-mute">No announcements posted yet.</p></div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-mute">Title</th>
                <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-mute">Priority</th>
                <th className="text-left p-4 text-[10px] font-bold uppercase tracking-widest text-mute">Published</th>
                <th className="text-center p-4 text-[10px] font-bold uppercase tracking-widest text-mute">Read by</th>
                <th className="p-4" />
              </tr>
            </thead>
            <tbody>
              {announcements.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/50" style={{ opacity: a.isActive ? 1 : 0.5 }}>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-mute">{publishedDate ? format(publishedDate, 'dd MMM yyyy') : '—'}</td>
                    <td className="p-4 text-center text-mute">{(a.readBy ?? []).length}</td>
                    <td className="p-4">
                      <div className="flex items-center gap-2 justify-end">
                        <button
                          ...
                          {a.pinned ? <PinOff size={14} style={{ color: '#8B8B85' }} /> : <Pin size={14} style={{ color: '#8B8B85' }} />}
                        </button>
                        <button
                          onClick={() => toggleAnnouncementActive(a.id, !a.isActive)}
                          className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors"
                          title={a.isActive ? 'Deactivate' : 'Activate'}>
                          {a.isActive ? <EyeOff size={14} style={{ color: '#8B8B85' }} /> : <Eye size={14} style={{ color: '#8B8B85' }} />}
                        </button>
                      </div>
                    </td>
                  </tr>
