...
- **Deploy when maintainer ships**: `deploy:rules` (unchanged — verify bind) → `gcloud run deploy pulse-api --source . --region asia-south1 --no-cpu-throttling` (server: stage1 sanitizer, master sanitizers, login SM/ASM auto-accumulate) → `npm run deploy` (hosting) → `verify:deploy`.
- **Sub-DSA (FAC-) payout decision still open** (the one genuine business call): the HRMS-`connectors` "Sub DSA" channel partner stays **attribution-only** (no automatic per-login payout) as built — Rahul to confirm or switch to an auto per-login payout later. Everything else above is structure-complete.
