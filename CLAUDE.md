...
- **Frontend**: `src/config/notifications.ts` (the 12-item `NOTIFICATION_TOGGLES` registry, keys must match the server) + `src/features/admin/NotificationSettingsPage.tsx` (standalone `/admin/notifications`, grouped toggle switches, super-admin edit / manager+admin view, live `onSnapshot`). Launcher header gains a **"Notifications"** link for super admins + managers/admins (next to "Shares"). New route `/admin/notifications`.
- tsc + build clean; no rules/index change.
