...
### Phase 2 — RESCOPED after analysis ✅ (2026-07-23): the trap is CLOSED; the real finding was a DPDP gap
**A read-only production audit settles it: the `deleted==false` trap no longer exists at the data level — 0 of 1546 leads are missing the field** (1521 old-model + 25 CRM 2.0; the 2026-07-20 backfill + create-path fix did their job, and all 9 CRM 2.0 create sites set `deleted:false`). **So the originally-scoped "remove the trap" sweep across ~14 queries would have been pure churn** — and, per the caution below, actively risky since those queries' model scoping changed once CRM 2.0 leads gained the field. **Do not do the mechanical swap.** What the analysis DID surface was a live compliance bug, now fixed:
- **RIGHT TO ERASURE MISSED EVERY CRM 2.0 LEAD** (`RightToBeForgottenPage` + `src/lib/leadAnonymisation.ts`). Search queried `phone`/`displayName` (old-model only), so a person existing only as a CRM 2.0 lead was **UNFINDABLE** — the request silently returned nothing; and `anonymiseLead()` cleared only `displayName`/`phone`/`email`/PAN, so a CRM 2.0 lead's `name`/`customerName`/`mobile` survived. **Every new lead is CRM 2.0**, so the gap widened daily.
- **Fixed for BOTH shapes**, including carriers that post-date the original helper and were never covered: `altPhones` + `phoneOriginal` (multi-number import + the phone-canonicalisation backfill), `importExtras` (every unmapped sheet column), `customerProfile`/`notes`/`nextFollowUpNote`/inline `activityLog`/`meetingLocation`, `dupeKeys` (phone/email-derived hashes), the **WhatsApp thread** (bodies are the customer's words; from/to are their number), and **the LEAD-LEVEL activity feed `/leads/{id}/activities`** (Phase P) — only `opportunities/*/activities` were walked, so call notes survived erasure on **every** lead, old-model included.
- Search now queries both field pairs + de-dupes, and **no longer filters `deleted==false`** — a soft-deleted record still holds PII and must stay erasable. Results render via **`leadName()`/`leadMobile()` from `src/lib/crm2/leadModel`** — the **first client adoption of the normalizer** (Phase 2's actual goal), so a CRM 2.0 lead shows its name/number instead of blank.
- **Dead code noted, NOT removed:** `checkForDuplicates` in `src/features/crm/leads/duplicateDetection.ts` has **zero callers** (superseded by the server-side `checkDuplicateServer` on 2026-06-30) and is also model-blind. Left in place pending an owner OK.
- **Second finding, same class — CLIENT reports + RM actuals excluded every CRM 2.0 lead** (fixed 2026-07-23, hosting-only). Confirmed against PRODUCTION, not inferred: of 25 CRM 2.0 leads, **22 have no `displayName` and no `primaryOwnerId`, while 24 DO have `assignedRm`**. Old-CRM Customers own by uid + name via `displayName`; CRM 2.0 leads own by **FAPL** (`assignedRm`) + name via `name`. Three client surfaces knew only the old shape — and so **disagreed with the Performance hub**, whose server-side equivalents were fixed on 2026-07-20:
  - **`LeadAgingPage`** — 22/25 listed as unnamed + "Unassigned", so a manager chasing stale leads saw phantom un-actionable rows. Now resolves the owner via `leadOwner()` through BOTH a uid→name and a **FAPL→name** map, and the name via `leadName()`.
  - **`useRmTargets`** — `computeActuals()` queried only `primaryOwnerId == uid`, so an RM's `newLeads` **undercounted against their target**; now two pinned queries (`primaryOwnerId == uid` + `assignedRm == fapl`). The team aggregation skipped CRM 2.0 leads outright (`if (l.primaryOwnerId && …)`); now attributes via `leadOwner()` + a **FAPL→uid** map so both models land in the same per-RM bucket.
  - **`CrmPerformanceWidget`** (HRMS employee profile) — same undercount. Opportunities still counted from old-model leads only, which is correct: the CRM 2.0 funnel uses cases.
  - All three now filter with **`isLeadDeleted()`** rather than `where('deleted','==',false)` — model-proof, and the trap cannot silently return if a create path ever misses the field again.
- **Remaining Phase 2 value is small and targeted:** adopt `leadModel` where a read is genuinely cross-model. Already converted: server `accumulatePerf` + activity summary (2026-07-20), RTBF, LeadAging, useRmTargets, CrmPerformanceWidget (2026-07-23). **Still old-model-by-design (correct, do not "fix"):** anything reading `opportunities` / imports / the Customers list itself.

### Phase 2 CAUTION (still applies to any future sweep) — the `deleted==false` "trap" is NOT a blind swap
...
- Verified: tsc clean, `eslint .` 0 errors and **no new warnings** (69 before and after — the generated import headers were pruned), 208 unit tests, build clean. **CI green (run 206, `5f26de5`)** — Typecheck · ESLint · Unit tests · Build · qa:meta · qa:sla · qa:queue · qa:partner all pass, confirming the 2026-07-22 gate-harness + stale-gate fixes hold under real back-to-back CI conditions (the partner gate had been red on EVERY push for 27 commits before them). #### Phase 4 page breakup #2 ✅ — `OpportunityDetailPage` 1450 → 472 (2026-07-23, hosting-only, verify:deploy 3/3)
Split on its natural seams; **verbatim, proven by a declaration-set diff (26 top-level declarations before, the same 26 after)**. **`stageForms.tsx` (614)** — the stage-data types + `LOAN_DOCS`/`TYPE_COLORS`/`TYPE_ICONS`/`ACTIVITY_ICONS` + local form primitives + all six per-stage capture forms · **`StageAdvanceModal.tsx` (172)** · **`StageDataHistory.tsx` (238)** · the page keeps `LostReasonModal` + `StageStepper`. eslint back to the 69-warning baseline (generated import headers pruned).
- **Also removed `checkForDuplicates()`** from `duplicateDetection.ts` (95→37): superseded by the server-side `checkDuplicateServer` on 2026-06-30 (the client query threw permission-denied for telecallers — the cause of the silent "Save Customer" failure) and **zero callers** since; it was also model-blind (old-CRM `phone`/`panRaw` only, so it could never match a CRM 2.0 lead). `checkDuplicateServer` + `DuplicateVerdict` stay; the file now records why the client version is gone.

#### Phase 4 page breakup #3 ✅ — `OffboardingPage` 1310 → 228 + the FnF math is now TESTED (2026-07-23, hosting-only, verify:deploy 3/3)
Verbatim (declaration-set diff: 20 before, 20 after). **`fnf.ts` (130)** · **`offboardingPdf.ts` (339)** — FnF statement / experience / relieving PDFs · **`offboardingModals.tsx` (378)** · **`ChecklistDetail.tsx` (304)** · the page keeps the list, filters and the shared badge/format helpers.
- **The reason for `fnf.ts` is not line count.** `computeFnF()` is the **one piece of real money arithmetic in the exit flow** (prorated salary · leave encashment capped at 30 days · gratuity at 5+ years · notice-period shortfall) and it was buried in a 1300-line page component, so it had **NO tests**. As a pure module it is testable and now has **13** (unit tests **208 → 221**), locking in CURRENT behaviour as a regression net: null when gross unset · daily rate = gross/workDays defaulting to 26 · 45 days of balance still pays only 30 · gratuity only at 5+ years and never with a missing date · notice deducts only the UNSERVED part and over-serving never becomes a bonus · net may legitimately go negative · optional extras omitted rather than zero.
- **Pattern worth repeating on the remaining pages:** when a big page hides pure calculation, extracting it is where the real value is — the split makes it testable, and the tests are what protect the number.

#### Phase 4 page breakup #4 ✅ — `CaseWorkspacePage` 1286 → 411 (2026-07-23, hosting-only, verify:deploy 3/3)
Verbatim (declaration-set diff: 19 before, 19 after). **`caseTabs.tsx` (397)** — Eligibility (stage 2) · Drive link (stage 3) · Collaboration · Client-ID · Details · **`caseStage1.tsx` (232)** · **`caseDocuments.tsx` (186)** — tracker + vault picker · **`caseApplicants.tsx` (126)** · the page keeps the 10-stage pipeline shell + stage routing. Shared `WithId`/`fmtTs`/`useSubcollection` are exported from the page.
- **Unlike the Offboarding split, there was NO buried calculation to rescue** — the page only DISPLAYS money (`inr`, gated on `payout.amounts.read`); the CRM 2.0 money math already lives in tested libs (`src/lib/crm2/payout.ts` · `logins.ts` · `slab.ts`) and the authoritative figures are server-calculated. Structure only; no new tests warranted. **Checking for that before splitting is the useful habit — not every big page hides logic.**
- Re-ran the **connector isolation gate 31/31** since the `cases` rules sit in its path.

#### Phase 4 page breakup #5 ✅ — `EmployeeProfilePage` 1192 → 350 (2026-07-23, hosting-only, verify:deploy 3/3)
Verbatim (declaration-set diff: 20 before, 20 after). **`profileModals.tsx` (436)** — the admin/HR "Edit details" + the employee's own "Edit My Details" · **`profileSections.tsx` (377)** — assets · identity verification · letters · completion banner · `FieldRow`/`Section` · **`useEmployeeProfile.ts` (82)** — the four Firestore reads (`users` · `employee_profiles` · `employee_sensitive` · `user_details`) · the page keeps the shell + photo upload. **Reads stay exactly as gated** — `employee_sensitive` (salary + bank) is admin/HR-only in rules and nothing here widens it.
- Checked for buried calculation first (the habit from #3): **none worth rescuing** — the only arithmetic is a completion percentage for a progress bar. Structure only.
- `ASSET_TYPE_LABELS` was moved INTO `profileSections` rather than exported back from the page, since the page no longer referenced it — sharing it would have created a pointless circular import.

**Phase 4 running total: 5 of 11 pages done.** MastersPage 1805→287 · OpportunityDetailPage 1450→472 · OffboardingPage 1310→228 (+13 FnF tests) · CaseWorkspacePage 1286→411 · EmployeeProfilePage 1192→350. Remaining: HrLetterGeneratorPage 1170 · AdminLeavePage 1132 · HrmsDashboardPage 1074 · AdminAttendancePage 1040 · ProbationPage 1035 · Crm2LeadsPage 1033.
**Method that has held for all five:** slice on comment-block-aware boundaries → let tsc enumerate the wiring → **verify with a declaration-set diff** (tsc alone won't tell you a component was silently dropped) → prune the generated import headers so the eslint baseline stays at 69.

#### Phase 4 page breakup #6 ✅ — `AdminLeavePage` 1132 → 99 + balance math TESTED (2026-07-23, hosting-only, verify:deploy 3/3)
**`leaveBalancesTab.tsx` (444)** · **`leaveApprovalTabs.tsx` (437)** · **`leaveEncashmentTab.tsx` (170)** · **`balanceEdit.ts` (85)** · the page keeps the tab shell + badge counts.
- **`balanceEdit.ts` is the point.** `/leave_balances` is the most bug-prone area of HRMS — **four** correctness fixes in June (approve seeding `total:0` · cancel not refunding · partial docs crashing readers · the calendar-vs-financial-year split) and one in July (keying by the LEAVE's FY, not the click date) — yet the admin balance editor's arithmetic sat inline in a modal save handler with **no tests**. Now **11** (unit tests **221 → 232**): remaining = total − used · **`used` is NEVER touched** (it derives from approved leave; overwriting would rewrite history) · remaining **clamps at 0** when a total is set below what's used · only CHANGED types produce an adjustment (no-op save writes nothing) · a **partial balance doc** works, a missing type starts all-zero (real case: a comp-off grant creates a doc with only `comp_off`) · a blank row is ignored rather than zeroing the type. **The modal calls the tested function**, so the tests guard the live path.
- **PROCESS NOTE (why the declaration-set diff is mandatory, not optional):** the first cut here **silently DROPPED `AllTab`** — the slice boundaries left a 144-line gap. tsc caught the dangling import and the diff confirmed the loss; the block was recovered from git and restored. A split can lose a whole component without any obvious symptom.

**Phase 4 running total: 6 of 11.** MastersPage 1805→287 · OpportunityDetailPage 1450→472 · OffboardingPage 1310→228 (+13 tests) · CaseWorkspacePage 1286→411 · EmployeeProfilePage 1192→350 · AdminLeavePage 1132→99 (+11 tests). Remaining: HrLetterGeneratorPage 1170 · HrmsDashboardPage 1074 · AdminAttendancePage 1040 · ProbationPage 1035 · Crm2LeadsPage 1033.

#### Phase 4 #7 — `HrLetterGeneratorPage`: a LIVE BUG found, page only partly reducible (2026-07-23, hosting-only, verify:deploy 3/3)
**The real find was not the split.** There were **TWO number-to-words implementations and neither was tested**: `src/lib/numberToWords.ts` `amountInWords()` (bank-login form) and a private `ctcToWords()` in the letter page. Algorithmically identical **except the letter copy lacked `Math.round` and the negative guard** — and the CTC field is free text read with `parseFloat`, never rounded. **So a decimal CTC (e.g. "1250000.50") left a fractional remainder, `units[0.5]` → `undefined`, and the offer/appointment letter printed "… Fifty Thousand undefined Only"** on a document an employee takes to a bank.
- `ctcToWords` **deleted**; letters call the shared rounding implementation via new **`amountInWordsWithOnly()`** (keeps the legal "Only" suffix). **`numberToWords.test.ts` added — 10 tests** (Indian scale · multi-scale composition · **the decimal case that used to break** · zero/negative/NaN). Unit tests **232 → 242**.
- Extracted `letterFormBits.tsx` (117) — catalogue + form-style helpers + `LetterRow`.
- **HONEST LIMIT: the page is 1170 → 1053 and STAYS over 1000.** It is ONE component holding **~60 interleaved `useState` values across five letter types**; `validate()` alone reads ~40 of them directly. The remaining bulk **cannot be moved mechanically** — splitting it means restructuring that state, which is a **redesign, not a refactor**, and not worth doing unilaterally on legal-document generation. **Flagged for a deliberate pass.**
- **Lesson worth carrying: the duplicated-implementation smell (Phase 1's `formatCurrency` problem) also applies to non-money helpers.** Two copies of the same algorithm WILL drift, and the drift showed up on a legal document.

#### Phase 4 #8 — `HrmsDashboardPage` 1074 → 273, and SATURDAY was not a working day in two screens (2026-07-23, hosting-only, verify:deploy 3/3)
**Finvastra works Mon–SATURDAY** — the leave system has enforced that since Phase F (`calculateWorkingDays` uses `d.getDay() !== 0`). But **two HRMS screens used date-fns `isWeekend()`**, which treats Saturday as a weekend:
1. **`HrmsDashboardPage`** — the monthly working-days figure was **~4 days SHORT** (July 2026: **23 instead of 27**), so the "present / working days" attendance stat used the wrong denominator.
2. **`AttendancePage`** — `isPastWorkDay` excluded Saturdays, so **an employee absent on a SATURDAY could not raise a regularization request at all.** A workflow blocked outright, not a display nit.
- **NEW `src/lib/workingDays.ts`** — `isWorkingDay()` + `workingDaysInMonth()`, matching `hooks/useLeave.ts` exactly (that hook is DO-NOT-TOUCH, keeps its copy, stays the reference). **9 tests** pin the rule: Saturday counts · only Sunday is off · a **Saturday** holiday is removed · a **Sunday** holiday is a no-op · leap February · 1-based months. Unit tests **242 → 251**.
- **Deliberately NOT unified with `src/lib/crm2/businessHours.ts`** — that encodes a DIFFERENT rule for a different purpose (CRM SLA working-time, where the **1st and 2nd Saturdays are off**). HRMS attendance/leave has no such carve-out. Two rules that look alike but aren't.
...
#### Phase 4 #10 ✅ — `ProbationPage` 1035 → 410 + countdown TESTED (2026-07-23, hosting-only, verify:deploy 3/3)
...
**crm2-whatsapp ✅ ROUTE-GROUP extraction resumed (2026-07-23, Cloud Run rev `pulse-api-00150-zn6`, verify:deploy 3/3)** — **the first true route GROUP out of crm2.ts**, deliberately chosen as the lowest-risk proof of the pattern: it mirrors the already-extracted `meta.ts` almost exactly and is **NOT on the money path**. **`server/crm2/whatsapp.ts` (277)** = `registerWhatsAppRoutes(app, requireSchedulerOrAdmin)` holding `GET|POST /api/webhooks/whatsapp` · `POST /api/crm2/whatsapp/send` · `POST /api/crm2/whatsapp/:leadId/read` · `POST /api/crm2/jobs/run-whatsapp-retry`. crm2.ts **4938 → 4705**.
- **CONTEXT THREADING — this is the pattern the remaining groups need.** `requireSchedulerOrAdmin` is **passed in**, not duplicated, because it closes over the `verifyScheduler` dependency `server.ts` injects into `registerCrm2Routes`. That is precisely why crm2.ts resisted the plain `registerXxx(app)` shape that worked for all 11 server.ts groups.
- **Kept intact deliberately (and the module comment says so):** the phone→lead lookup stays INSIDE the minting transaction, so two concurrent first messages from one number cannot double-mint a lead (fixed 2026-07-02). Moving it out would look like a harmless tidy-up.
- **Verified with the FULL gate suite, not just tsc** (server code): meta 15/15 · connector 31/31 · partner 32/32 · queue 18/18 · sla 17/17 · **MONEY 13/13** · 257 unit tests · live smoke (webhook 403 without a verify token, send/retry 401).

**crm2-connectorRoutes ✅ (2026-07-23, Cloud Run rev `pulse-api-00151-7sh`, verify:deploy 3/3)** — second route group, still off the money path. **`server/crm2/connectorRoutes.ts` (317)** = `registerConnectorRoutes(app)`: `POST /api/crm2/connectors` (mint CON-###, encrypt PAN + bank) · `PATCH /api/crm2/connectors/:id` (screening/assessment/onboarding) · `GET|PATCH /api/crm2/partner-scoring-config` · `POST /api/crm2/admin/migrate-connector-codes`. crm2.ts **4705 → 4441**.
- **`EMPTY_ONBOARDING` / `nextConnectorCodeServer` / `getPartnerRubric` are hoisted out of the register fn and EXPORTED** — crm2.ts still needs all three when a lead is promoted into the partner funnel (`createPartnerCandidate` mints a code, reads the rubric, seeds the same checklist).
- **Deliberately NOT taken: `mappings`,** even though it sits right beside this block. **`resolveMapping` is on the DISBURSE path with four call sites** — it belongs with the money extractions, done one at a time with the money gate, not bundled into a partner-funnel move.
- **PROCESS NOTE — prune per-file.** The first prune diffed eslint warnings across BOTH files at once, so symbols unused in `connectorRoutes.ts` but still used in crm2.ts (`reqStr`/`strArr`/`optBool`/`optEnum`) looked newly dead and were removed; tsc caught it instantly (35 errors). Net result **68 warnings — one BELOW the long-standing 69 baseline**, since the move also retired a genuinely dead symbol.
- Verified: partner 32/32 · connector 31/31 · meta 15/15 · **MONEY 13/13** · 257 unit tests · live smoke (all three moved routes 401).

**crm2-mastersClients ✅ (2026-07-23, Cloud Run rev `pulse-api-00152-l6q`, verify:deploy 3/3)** — third route group, still off the money path. **`server/crm2/mastersClients.ts` (203)** = `registerMastersClientRoutes(app)`: `POST|PATCH /api/crm2/masters/:type` (generic master CRUD) · `POST /api/crm2/admin/migrate-aggregator-ids` · `POST|PATCH /api/crm2/clients[/:id]` (Client Master, FCL-YYYY-#####). The **`MASTERS` registry + `masterCfg()` moved with it** — verified used by nothing else before cutting. crm2.ts **4441 → 4273**.
- **STOPPED DELIBERATELY AT `resolveMapping`.** The mappings routes sit immediately after the clients block and were tempting to take in the same pass, but `resolveMapping` is read by the **DISBURSE path (4 call sites)** — mappings goes with the money extractions, each behind the money gate.
- **Caught while cutting:** the `registerConnectorRoutes(app)` call added in the previous commit sat inside this block and was swept into the new module. Both register calls now live together in crm2.ts, and **each of the three groups is registered exactly once** (grep-verified — the same check that caught stray register calls during the server.ts split).
- Verified: **MONEY 13/13** · partner 32/32 · connector 31/31 · queue 18/18 · 257 unit tests · live smoke (all three moved routes 401). eslint **68** — still one below the 69 baseline.

**crm2-queueRoutes ✅ (2026-07-23, Cloud Run rev `pulse-api-00153-tg9`, verify:deploy 3/3)** — fourth route group, still off the money path. **`server/crm2/queueRoutes.ts` (182)** = `registerQueueRoutes(app, loadSlaConfig, loadBusinessHours, resolveEscalationUids, notify)`: `POST /api/crm2/queue/claim` (oldest unassigned lead, claimed in a tx) · `POST /api/crm2/queue/release` (return to pool, preserve `receivedAt`, flag at ≥3) · `GET /api/crm2/queue/state`. The four queue helpers (`loadQueues`/`callerQueueSkills`/`CRM2_TERMINAL_STATUS`/`isWaiting`) moved with it. crm2.ts **4273 → 4131**.
- **FOUR deps threaded in** (widest threading so far): `loadSlaConfig` + `loadBusinessHours` (shared with the SLA sweep still in crm2.ts) and `resolveEscalationUids` + `notify` (registerCrm2Routes-local; `notify` closes over the injected `sendBrandedEmail`). The release route escalates to managers when a lead is released ≥3 times, so it needs both. Registered **after** those helpers are defined.
- Prune per-file; `elapsedWorkingMs`/`DEFAULT_BUSINESS_HOURS` **stay** imported in crm2.ts (the SLA sweep uses them). Back at the **69** baseline.
- Verified: queue 18/18 · sla 17/17 · **MONEY 13/13** · 257 unit tests · live smoke (all three moved routes 401).

**crm2-taskRoutes ✅ (2026-07-23, Cloud Run rev `pulse-api-00154-rxp`, verify:deploy 3/3)** — fifth route group, off the money path (case metadata + `crm_tasks`, never payout/misRecords/disburse). **`server/crm2/taskRoutes.ts` (291)** = `registerTaskRoutes(app, notify, sendBrandedEmail, faplToUid, faplDisplayName)`: `POST /api/crm2/cases/:id/collaborators` · `POST|PATCH /api/crm2/cases/:id/tasks[/:taskId]` · `GET /api/crm2/my-case-tasks` · `POST|PATCH /api/crm2/tasks[/:id]`. `notifyFapls` moved with it. crm2.ts **4131 → 3873**.
- **tsc caught a threading typo:** I first mistyped the `EmailFn` param as `{ intro; rows; cta }`; the real `Deps.sendBrandedEmail` is `{ title; intro; rows; note?; ctaLabel?; ctaLink? }` (the task-assigned email passes `title`). Copied the Deps signature verbatim into the module's local type so it can't drift.
- Verified with the **DEDICATED `crm2-phase6-gate` 10/10** (collaborators deduped + handlingRm-stripped · non-owner 403 · task surfaces in my-case-tasks cross-case · mark-done drops it) · **MONEY 13/13** · 257 unit tests · 69 baseline · live smoke 401.

**crm2-leadRoutes ✅ (2026-07-23, Cloud Run rev `pulse-api-00155-4gf`, verify:deploy 3/3)** — the LAST non-money group, and the largest (567 lines). **`server/crm2/leadRoutes.ts` (571)** = `registerLeadRoutes(app, faplToUid, notify)`: `POST|PATCH /api/crm2/leads[/:id]` · `POST /api/crm2/leads/:id/convert` · `POST /api/crm2/leads/:id/promote` · `POST /api/crm2/admin/backfill-lead-codes`. **Convert VERIFIED off the money path before cutting** — it initialises `amountSanctioned/Disbursed = null`, `payoutStatus = NOT_DUE`, and never calls `resolveSlab`/`resolveMapping` or writes a payout cycle. crm2.ts **3873 → 3310**.
- **Two shared foundations moved:** `server/crm2/leadEnums.ts` (NEW) — `LEAD_CATEGORIES`/`SOURCES`/`STATUSES` + `DROP_REASONS` were file-wide local consts (LEAD_CATEGORIES is used by public intake too, which stays in crm2.ts) · **`expandDocTracker` moved into `context.ts`** (only needs `createAudit`) and is now imported by BOTH crm2.ts (walk-in case-open) and leadRoutes (convert) — a pure doc-tracker seeder, cleaner as a shared export than threaded.
- **Left in crm2.ts (interleaved, own sub-domains):** public intake · Meta webhook · SLA sweep · partner-candidate routes — only the clean CRUD block moved.
- **Widest gate coverage yet** (convert touches case creation): phase2 (convert) 31/31 · phase3 **12/13** [the 1 fail is the documented env-only GCS vault-upload — confirmed by reading the assertion, NOT a regression] · **MONEY 13/13** · partner 32/32 · 257 unit tests · 69 baseline · live smoke 401.

**crm2 route groups extracted (6): whatsapp · connectorRoutes · mastersClients · queueRoutes · taskRoutes · leadRoutes. crm2.ts 5721 → 3310 (~42% out).** **All non-money groups are done.**

**crm2-mappingRoutes ✅ FIRST money-path group (2026-07-23, Cloud Run rev `pulse-api-00156-kzw`, verify:deploy 3/3)** — DSA-code mapping + payout-slab ADMIN CRUD. **`server/crm2/mappingRoutes.ts` (193)** = `registerMappingRoutes(app)`: `POST|PATCH /api/crm2/mappings[/:id]` · `POST /api/crm2/mappings/:id/slabs` · `GET /api/crm2/mappings/:id/resolve-slab` (disburse-dialog preview) · `POST /api/crm2/mappings/:id/slabs/:slabId/end`. crm2.ts **3310 → 3148**.
- **CRITICAL: `resolveMapping()` — the disburse reader (4 call sites) — DELIBERATELY STAYS in crm2.ts, untouched.** These routes don't use it; they use only the tested slab libs (`slab.js` resolveSlab, `slabs.js` sanitizeSlab/toResolution/assertNoOverlaps), so nothing is threaded. A clean extraction despite being money-adjacent.
- Verified with the money gates: **MONEY 13/13 · phase4 24/24** (disburse→cycle+MIS · frozen economics · out-of-order milestone ±override · slab resolution) · 257 unit tests · 69 baseline · live smoke 401.

**crm2-caseRoutes ✅ SECOND money-path group (2026-07-24, Cloud Run rev `pulse-api-00157-8gs`, verify:deploy 3/3 + cases/stage/patch unauth 401)** — the 6 case-level routes (923–1393). **`server/crm2/caseRoutes.ts` (492)** = `registerCaseRoutes(app)`: `POST /api/crm2/cases` (walk-in open) · `PATCH /api/crm2/cases/:id` · `POST /api/crm2/cases/:id/stage` · `POST|PATCH|DELETE /api/crm2/cases/:id/applicants[/:applicantId]` · `PATCH /api/crm2/cases/:id/doc-tracker/:rowId`. **Cases is OFF the money path** (verified before cutting — open only null-inits `payoutStatus: NOT_DUE`; no `resolveSlab`/`resolveMapping`/cycle writes). Moved `StageTrackerRow` + the case-local sanitizers (`sanitizeApplicant`/`sanitizeStage1`/`sanitizeEligibility`); `expandDocTracker` was already hoisted to `context.ts` (shared by case-open + lead convert). **Deleted a dead `readTrackerRows` helper (0 call sites) that rode along.** crm2.ts **3148 → 2677 (~53% out)**. Gates: MONEY 13/13 · phase4 24/24 · phase4a **11/11** · phase3 12/13 (env-only GCS vault) · 257 unit tests · 69 baseline.
- **STALE-GATE FIX (2nd gate-rot incident, after the partner gate):** `.qa/crm2-phase4a-gate.mjs` advanced a login FILE_LOGIN→CODE_LOGIN_DONE **without** confirming "Docs sent to bank" — but the login stage endpoint has 422'd without it since **2026-06-22** (the gate last changed `599cc9a`, before that rule). Diagnosed by running the UNMODIFIED committed HEAD against the gate → identical 10/4 fail, proving it predates this change (not a regression). Added the `docsSent: true` PATCH before the advance loop → 11/11. **Lesson stands: a gate that only runs locally rots against deliberate product rules; when a stage-gate rule ships, update its gate the same commit.**

**crm2-loginRoutes ✅ THIRD money-path group (2026-07-24, Cloud Run rev `pulse-api-00158-gl6`, verify:deploy 3/3 + logins open/patch/stage unauth 401)** — the 3 per-login PIPELINE routes (949–1137). **`server/crm2/loginRoutes.ts` (233)** = `registerLoginRoutes(app)`: `POST /api/crm2/cases/:id/logins` (open, mints `LGN-YYYY-####`, bumps case→IN_PROGRESS on first login) · `PATCH .../logins/:loginId` (data edit, decoupled from stage; SM/ASM auto-accumulate into the lender master) · `POST .../logins/:loginId/stage` (advance forward-by-one via `validateLoginTransition`; docs-sent gate; early-close outcome). Moved the login consts (`SUB_PROCESS_KEYS`/`emptySubProcess(es)`/`LOGIN_EDITABLE`/`LOGIN_PROTECTED`). **OFF the money path** — open only null-inits `payoutStatus: NOT_DUE`; the per-login **DISBURSE** money engine (uses `resolveMapping`/`resolveSlab`/`payoutCycles`/`misRecords`) DELIBERATELY stays in crm2.ts. crm2.ts **2677 → 2465 (~57% out)**. Pruned dead `logins.js` + `reqEnum`/`strArr` imports → eslint 69. Gates: phase4a **11/11** (open/patch/stage, forward-by-one, DISBURSED-reserved 422, early-close) · MONEY **13/13** (disburse→cycle+MIS, frozen economics, milestone lock-step, Sub-DSA auto-payout) · build clean.

**crm2 route groups extracted (9). crm2.ts 5721 → 2465 (~57% out).** Remaining money-path (heaviest, one at a time, money gate each, STOP on any non-13/13): **disburse/payout-cycles · mis · recon · jobs.** NOTE: disburse/payout-cycles/MIS may not be cleanly separable (they share the disburse tx + `resolveMapping`/`resolveSlab` + `misRecords`/`payoutCycles` writers) — if a clean cut isn't safe, leave them together rather than force it. **The per-case + per-login DISBURSE, disburse-preview, and milestone PATCH are the money core — they + `resolveMapping`/`resolveSlab` should move together (or stay together) as one unit.**
**Remaining groups, money LAST:** non-money first (masters · admin-migrate-aggregator · clients · queue · tasks · leads), then the money path one at a time with the money gate on each (**mappings/resolveMapping · cases · logins · disburse/payout-cycles · mis · recon · jobs**). The remaining reduction is the per-domain route-group extraction (scattered routes 116–1128+ interleaved by domain + shared db-bound helpers like `resolveMapping`/`findDuplicate`/`masterCfg`/the meta+whatsapp processors) — a heavy, dedicated multi-commit effort, foundation fully in place (`core`/`context`/`sanitizers`/`slabs`/`connectors` all importable; only `verifyScheduler`+`sendBrandedEmail` need threading, both already in `server/lib/*`). **Remaining pure clusters to hoist next: `sanitizeApplicant`/`sanitizeCustomerProfile`/`sanitizeCycle`/`sanitizeEligibility`/`sanitizeTask*` + `sanitizeSlab` + `masterCfg`/`nextConnectorCodeServer` + `connectorMainFields`/`buildPayoutBank` + the meta/whatsapp parsing helpers; then the Deps-context-threaded route-group extraction (full gate suite).**
...
4. **Mark the account as a connector** — HRMS → **Permission Manager → "Partner (connector) logins"** → pick the account + the CON- record → **Make this a partner login**. This forces `hrmsAccess:false` + `crmAccess:false`, sets `perms:{crm.leads.write:true}`, and re-stamps claims. _(Equivalent API: `POST /api/admin/users/{uid}/connector` `{"connectorId":"CON-###"}`.)_
...
- **To revoke:** same section → **Revoke** (confirms first), then deactivate the account via HRMS → Employees → Mark as Exited. Their leads/cases/payout history are untouched.
- **STILL PENDING: a production dry run.** Everything above is proven in the emulator; **no real connector account has been created in production yet.** Before the first real partner is onboarded, walk one test account end-to-end (create → link → sign in → submit a lead → confirm they see only their own case/payout/last-4 → revoke). Step 2 (the Workspace email) is a human step.

### Connector — admin UI + fail-closed hrmsAccess ✅ (2026-07-23, ruleset `9e6da756`, Cloud Run rev `pulse-api-00149-fbf`, verify:deploy 3/3)
- **NEW `src/features/hrms/admin/ConnectorAccountsSection.tsx`**, mounted on Permission Manager (super-admin) — **step 4 is now a button.** Deliberately kept OUT of the dense permissions table: making a partner login is a rare, deliberate, security-sensitive action, and its own section also gives an **audit view that did not exist — WHO the partner logins are and which CON- record each is tied to.** Pick account + pick CON- record → "Make this a partner login"; Revoke has a confirm. The component **never writes the user doc directly** — all flag-setting stays server-side in `POST /api/admin/users/:uid/connector`.
- **`hrmsAccess` now FAILS CLOSED.** It defaulted TRUE when a user doc omitted it, in **both** the rules fallback and the claim stamp — an access flag defaulting OPEN is backwards, and it was the one thing between a mis-created partner account and the employee directory. **Verified safe before flipping (read-only prod audit): all 26 user docs set it explicitly (23 true / 3 false), so nobody relied on the default, and every creation path writes it** (employee create · self-bootstrap in `AuthContext` · the connector endpoint). Changed `firestore.rules` `.get('hrmsAccess', true)` → `false` and the two claim stamps `?? true` → `=== true`. **The employee create path still defaults TRUE — correct for staff, and exactly why a partner account must set it explicitly rather than omit it.**
- Verified: tsc · eslint 0 errors · 208 tests · build · rules compile · **all five gates (connector 31/31 · partner 32/32 · sla 17/17 · queue 18/18 · meta 15/15)**.
- **Onboarding step 4 is now:** Permission Manager → *Partner (connector) logins* → pick the account + the CON- record → **Make this a partner login**. (The API call remains available and is what the button uses.)

### Connector — REMAINING WORK
1. ~~**No admin BUTTON yet for step 4**~~ **DONE (2026-07-23)** — superseded — the endpoint exists and is the supported path, but Permission Manager has no UI for it, so today it is a one-line API call. Worth adding a "Make this a partner account" control next to the CRM-role picker.
