...
- **Deploy**: merged to main; `deploy:rules` (new `tasks` block) → `deploy:indexes` (new `tasks.assignedTo` CG override — wait READY) → `gcloud run deploy pulse-api --source . --no-cpu-throttling` (**revision `pulse-api-00053-…`**) → `npm run deploy` (hosting) → `verify:deploy`.
