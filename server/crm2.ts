...
import { registerCaseRoutes } from "./crm2/caseRoutes.js";
import { registerLoginRoutes } from "./crm2/loginRoutes.js";
import {
...
export function registerCrm2Routes(app: express.Express, { db, admin, verifyScheduler, sendBrandedEmail }: Deps): void {
  ...
  // Phase 3 case lifecycle (CRUD / stage machine / applicants / docTracker).
  // Off the money path; disburse (Phase 4) stays in crm2.ts.
  ...
  // ═══ Phase 4 — Logins (per-login pipeline) — extracted to ./crm2/loginRoutes.ts
  // (2026-07-24). OPEN/PATCH/STAGE only; the per-login disburse money engine
  // stays below (needs resolveMapping/resolveSlab).
