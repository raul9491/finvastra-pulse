...
### Full security + cost audit — non-breaking fixes applied, NOT YET DEPLOYED (2026-07-20)
A three-front read-only audit (backend endpoints · firestore/storage rules + secrets · cloud-cost leakage). **Only strictly non-breaking fixes were applied** (verified: `npm run lint` exit 0; only `firestore.rules`, `server.ts`, `server/crm2.ts` changed). **⚠️ NONE ARE LIVE until the maintainer deploys** — `deploy:rules` for the rules fix + `gcloud run deploy pulse-api --no-cpu-throttling` for the server changes. Until then the CRITICAL leak below is still open in prod.
